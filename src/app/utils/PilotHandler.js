...
const pilotHandler = async () => {
  ...
  if (messages?.length > 0 && cacheValue && arraysEqual(cacheValue.departing, pilotList.departing) && arraysEqual(cacheValue.arriving, pilotList.arriving)) {
  ...
  if (pilotList.departing?.length === 0 && pilotList.arriving?.length === 0) {
    ...
    for (let i = 0; i < embeds.length; i += 1) {
      ...
      await pilotChannel.send(embeds[i].setFooter(`${client.user.username} • Message ${i + 1} of ${embeds.length} • Updated at ${now.format('HH:mmz')}`));
    }
  }
